...
// v2 namespaces isolate this deployment from the first Netlify build.
const rooms=getStore("easy-second-chat-rooms-v3");
const directory=getStore("easy-second-chat-directory-v3");
...
async function verifyPassword(password,r){try{
  if(!r||typeof r!=="object"||r.alg!=="PBKDF2-SHA256")return false;
  const salt=unb64(r.salt);
  const iterations=Number(r.iterations);
  if(salt.length<16||!Number.isInteger(iterations)||iterations<1000||iterations>2000000)return false;
  return equal(String(r.hash||""),await derive(password,salt,iterations));
}catch{return false}}
...
async function verifyStoredPassword(password,r){
  // Current format: { alg, iterations, salt, hash }
  if(r&&typeof r==="object"&&r.alg==="PBKDF2-SHA256")return {valid:await verifyPassword(password,r),upgrade:false};
  // Older PBKDF2 records may omit the alg field but still contain the full parameters.
  if(r&&typeof r==="object"&&r.salt&&r.hash&&r.iterations)return {valid:await verifyPassword(password,{...r,alg:"PBKDF2-SHA256"}),upgrade:true};
  // Older Netlify/Cloudflare builds stored a raw SHA-256 string.
  if(typeof r==="string"){
    return {valid:equal(r,await legacyPasswordHash(password)),upgrade:true};
  }
  // Some older builds used { passwordHash: "..." }.
  if(r&&typeof r==="object"&&typeof r.passwordHash==="string"){
    return {valid:equal(r.passwordHash,await legacyPasswordHash(password)),upgrade:true};
  }
...
function canonicalRoom(room){return clean(room,MAX_ROOM).normalize("NFKC").toLowerCase()}
async function roomKey(room){
  const bytes=new TextEncoder().encode(canonicalRoom(room));
  const digest=await crypto.subtle.digest("SHA-256",bytes);
  return "room:"+b64(new Uint8Array(digest));
}
async function getRoom(room){
  return await rooms.get(await roomKey(room),{type:"json",consistency:"strong"});
}
async function saveRoom(room,data){
  await rooms.setJSON(await roomKey(room),data);
}
async function freshRoom(room,data){return !!(data&&data.roomName&&canonicalRoom(data.roomName)===canonicalRoom(room)&&Number.isFinite(Number(data.createdAt))&&Number(data.createdAt)+ROOM_RESET_MS>Date.now()&&(data.password||data.passwordHash)&&data.generation)}
function timeNow(){return new Date().toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false})}
async function authRoom(room,rt){const r=await getRoom(room);if(!freshRoom(room,r))return null;const s=secret();if(!s)return null;const ok=await verifyToken(rt,`${canonicalRoom(room)}.${r.generation}`,s);return ok?r:null}
...
async function directoryRegister(r){
  await directory.setJSON(await roomKey(r.roomName),{
    name:r.roomName,
    createdAt:r.createdAt,
    expiresAt:r.createdAt+ROOM_RESET_MS
  });
}
async function directoryRemove(room){await directory.delete(await roomKey(room))}
async function roomList(q){const out=[];let cursor;do{const page=await directory.list({prefix:"room:",cursor,limit:100});for(const x of page.blobs){const v=await directory.get(x.key,{type:"json",consistency:"strong"});if(v&&v.expiresAt>Date.now()&&(!q||v.name.toLowerCase().includes(q.toLowerCase())))out.push(v);else if(v)await directory.delete(x.key)}cursor=page.cursor}while(cursor);out.sort((a,b)=>a.name.localeCompare(b.name,"ko"));return out.slice(0,100)}
...
export default async (req)=>{
 ...
 try{
  ...
  if(path==="/room-create"&&req.method==="POST"){
   ...
   const old=await getRoom(room);
   if(freshRoom(room,old))return json({ok:false,error:"이미 존재하는 방입니다. 방 들어가기를 이용하세요."},409);
   if(old)await rooms.delete(await roomKey(room));
   ...
   const created=await rooms.set(await roomKey(room),JSON.stringify(r),{onlyIfNew:true});
   if(!created.modified){
     const current=await getRoom(room);
     if(freshRoom(room,current))return json({ok:false,error:"이미 존재하는 방입니다. 방 들어가기를 이용하세요."},409);
     return json({ok:false,error:"방 생성 중 충돌이 발생했습니다. 잠시 후 다시 시도하세요."},409);
   }
   await directoryRegister(r);
   return json({ok:true,room,createdAt,expiresAt:createdAt+ROOM_RESET_MS});
  }
  if(path==="/room-login"&&req.method==="POST"){
   const b=await getBody(req),room=clean(b.room,MAX_ROOM),password=String(b.password||"");
   ...
   const storedPassword=r.password ?? r.passwordHash;
   if(storedPassword==null)return json({ok:false,error:"이 방의 비밀번호 정보가 없습니다. 방을 새로 만들어 주세요."},500);
   const passwordCheck=await verifyStoredPassword(password,storedPassword);
   if(!passwordCheck.valid)return json({ok:false,error:"방 비밀번호가 올바르지 않습니다."},401);
   if(passwordCheck.upgrade){
     r.password=await hashPassword(password);
     delete r.passwordHash;
     await saveRoom(room,r);
   }
   const s=secret();
   if(!s)return json({ok:false,error:"ROOM_AUTH_SECRET가 설정되지 않았습니다."},503);
   const rt=await token(`${canonicalRoom(room)}.${r.generation}`,s,TOKEN_TTL_MS);
   return json({ok:true,token:rt,expiresInMs:TOKEN_TTL_MS});
  }
  ...
  console.error("EASY_SECOND_CHAT_API_ERROR",{path,method:req.method,message:e?.message,stack:e?.stack});
  const code=path==="/room-exists"?"ROOM_EXISTS":path==="/room-create"?"ROOM_CREATE":path==="/room-login"?"ROOM_LOGIN":path==="/state"?"STATE":path==="/send"?"SEND":path==="/rooms"?"ROOM_LIST":path==="/leave"?"LEAVE":"API";
  return json({ok:false,error:`서버 내부 오류가 발생했습니다. (${code})`},500)
 }
};
