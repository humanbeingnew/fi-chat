...
// 새 방은 느린 PBKDF2-HMAC-SHA256으로 저장합니다. 무료 Workers의 낮은 CPU 한도를 고려해 20,000회로 설정했습니다. 기존 방은 성공 로그인 시 자동 업그레이드합니다.
const PASSWORD_KDF_ITERATIONS = 20000;
...
async function verifyRoomPassword(password, record) {
  ...
  try {
    ...
    if (salt.length < 16 || !Number.isInteger(iterations) || iterations < 1000 || iterations > 2000000) {
      return false;
    }
...
<style>
*{box-sizing:border-box}body{margin:0;background:#0f1218;color:#eef2f7;font-family:Arial,sans-serif}.wrap{max-width:1050px;margin:24px auto;padding:14px}.card{background:#181d26;border:1px solid #303847;border-radius:18px;overflow:hidden;box-shadow:0 12px 40px #0006}header{padding:20px;border-bottom:1px solid #303847}h1{margin:0 0 5px}.sub{color:#9da8b8}.screen{padding:28px;max-width:760px;margin:auto}.homeButtons{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:24px}.homeButton{padding:24px 16px;font-size:18px;background:#202a3b;border:1px solid #3a4658}.homeButton span{display:block;font-size:32px;margin-bottom:10px}.formBox{display:grid;gap:10px;margin-top:18px}input,button{font:inherit;border-radius:10px;padding:12px;border:1px solid #3a4658}input{background:#0d1015;color:#fff}button{background:#4d7cff;color:#fff;border:0;cursor:pointer}.secondary{background:#303949}.danger{background:#8d3c4a}.back{margin-top:12px}.status{min-height:22px;color:#ffca79;margin-top:8px}.roomSearch{margin-top:20px;padding-top:16px;border-top:1px solid #303847}.roomSearchRow{display:flex;gap:8px}.roomSearchRow input{flex:1}.roomList{margin-top:8px;display:grid;gap:6px;max-height:220px;overflow:auto}.roomItem{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 11px;background:#202735;border-radius:9px;color:#dbe3ee}.roomItem button{padding:7px 10px;background:#35445f}.empty{color:#8793a6}.modal{position:fixed;inset:0;background:#0009;display:none;align-items:center;justify-content:center;padding:20px;z-index:20}.modalBox{width:min(420px,100%);background:#181d26;border:1px solid #3a4658;border-radius:16px;padding:22px;box-shadow:0 20px 60px #0008}.modalBox h2{margin:0 0 8px}.modalActions{display:flex;gap:8px;margin-top:12px}.modalActions button{flex:1}#chat{display:none;grid-template-columns:220px 1fr;min-height:590px}aside{padding:18px;background:#131821;border-right:1px solid #303847}#members{line-height:1.9;color:#cbd3df}.roomtag{margin:10px 0;padding:8px;border-radius:8px;background:#202735;color:#aeb9c9}main{display:flex;flex-direction:column;min-width:0}#messages{flex:1;overflow:auto;padding:18px;max-height:500px}.msg{padding:10px 12px;margin:8px 0;background:#222a38;border-radius:12px;overflow-wrap:anywhere}.msg b{color:#91b1ff}.msg small{color:#8793a6;margin-left:8px}.system{opacity:.7}.form{display:flex;gap:8px;padding:14px;border-top:1px solid #303847}.form input{flex:1;min-width:0}@media(max-width:680px){.homeButtons{grid-template-columns:1fr}#chat{grid-template-columns:1fr}aside{border-right:0;border-bottom:1px solid #303847}}
</style>
</head>
<body><div class="wrap"><div class="card">
<header><h1>EASY SECOND CHAT V2 💬</h1><div class="sub">방을 만든 뒤 친구들과 비밀번호로 입장할 수 있습니다.</div></header>

<section id="home" class="screen">
  <h2>무엇을 할까요?</h2>
  <div class="sub">처음 사용하는 경우에는 방을 만들고, 이미 방이 있다면 입장하세요.</div>
  <div class="homeButtons">
    <button class="homeButton" onclick="showScreen('create')"><span>➕</span>방 만들기</button>
    <button class="homeButton" onclick="showScreen('join')"><span>🚪</span>방 들어가기</button>
  </div>
  <div id="homeStatus" class="status"></div>
  <button type="button" class="secondary" onclick="health()">서버 상태 확인</button>
</section>

<section id="create" class="screen" style="display:none">
  <h2>방 만들기</h2>
  <div class="sub">새 방의 이름과 비밀번호를 설정하세요. 새 비밀번호는 12자 이상이어야 합니다.</div>
  <div class="formBox">
    <input id="createRoom" maxlength="40" placeholder="방 이름">
    <input id="createPassword" type="password" maxlength="128" placeholder="방 비밀번호 (12자 이상)">
...
<section id="join" class="screen" style="display:none">
  <h2>방 들어가기</h2>
  <div class="sub">닉네임과 이미 만들어진 방 이름을 입력하세요. 비밀번호는 다음 단계에서 확인합니다.</div>
  <div class="formBox">
    ...
    <button onclick="prepareJoin()">방 입장</button>
...
<div id="passwordModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="passwordTitle">
  <div class="modalBox">
    <h2 id="passwordTitle">방 비밀번호</h2>
    <div class="sub">선택한 방의 비밀번호를 입력하세요.</div>
    <div class="formBox">
      ...
      <div class="modalActions"><button type="button" class="secondary" onclick="closePasswordModal()">취소</button><button type="button" onclick="confirmJoinPassword()">확인</button></div>
    </div>
  </div>
</div>
...
let pendingCreatedPassword="";
const $=id=>document.getElementById(id);

function status(t){$("homeStatus").textContent=t}
function setJoinStatus(t){$("joinStatus").textContent=t}
function setCreateStatus(t){$("createStatus").textContent=t}

function showScreen(name){
  ["home","create","join"].forEach(id=>$(id).style.display=id===name?"block":"none");
  if(name!=="join") closePasswordModal();
  if(name==="create") setCreateStatus("");
  if(name==="join") setJoinStatus("");
}
...
function clearConnectionState(){clearTimeout(reconnectTimer);reconnectTimer=null;connecting=false}
...
function connect(){
  ...
  if(adminToken)wsUrl.searchParams.set("adminToken",adminToken);

  setJoinStatus("채팅 서버에 연결 중...");
  const currentWs=new WebSocket(wsUrl.toString());
  ws=currentWs;

  currentWs.onopen=()=>{
    if(ws!==currentWs)return;
    connecting=false;clearTimeout(reconnectTimer);setJoinStatus("연결 성공");
    $("home").style.display="none";$("create").style.display="none";$("join").style.display="none";$("chat").style.display="grid";
  ...
  currentWs.onmessage=e=>{
    ...
    if(d.type==="init"){token=d.token;renderMembers(d.members||[]);$("messages").innerHTML="";(d.messages||[]).forEach(addMsg)}
    else if(d.type==="members")renderMembers(d.members||[]);
    else if(d.type==="message"||d.type==="system")addMsg(d);
    else if(d.type==="error")addMsg({system:true,time:d.time||"",text:d.message||"오류가 발생했습니다."});
  };
  ...
  currentWs.onclose=e=>{
    ...
    if(manuallyLeft)return;
    if(adminToken){clearConnectionState();adminToken="";token="";setJoinStatus("관리자 WebSocket 연결이 종료되었습니다. 다시 관리자 로그인해 주세요.");$("join").style.display="block";$("chat").style.display="none";$("adminBox").style.display=isProtectedNameClient(me)?"block":"none";return;}
    setJoinStatus("WebSocket 연결 종료: 코드 "+e.code+(e.reason?" / "+e.reason:"")+" (1.5초 후 재연결)");scheduleReconnect();
  };
  currentWs.onerror=()=>{if(ws!==currentWs)return;setJoinStatus("WebSocket 연결 오류. 자동 재연결을 시도합니다...")};
}
...
function checkProtectedName(){$("adminBox").style.display=isProtectedNameClient($("name").value)?"block":"none"}

async function createRoom(){
  const roomName=$("createRoom").value.trim();
  const password=$("createPassword").value;
  if(!roomName||!password){setCreateStatus("방 이름과 비밀번호를 입력하세요.");return}
  if(password.length<12){setCreateStatus("새 방 비밀번호는 최소 12자 이상이어야 합니다.");return}
  setCreateStatus("방을 만드는 중...");
  try{
    const r=await fetch("/api/room-create",{method:"POST",headers:{"content-type":"application/json"},cache:"no-store",body:JSON.stringify({room:roomName,password})});
    let d={};try{d=await r.json()}catch{}
    if(!r.ok)throw new Error(d.error||"방을 만들지 못했습니다.");
    pendingCreatedPassword=password;
    $("room").value=roomName;
    $("createPassword").value="";
    $("name").value="";
    showScreen("join");
    setJoinStatus("방이 만들어졌습니다. 닉네임을 입력하고 입장하세요.");
    $("name").focus();
  }catch(e){setCreateStatus("방 만들기 실패: "+e.message)}
}

function openPasswordModal(){
  $("roomPassword").value=pendingCreatedPassword||"";
  $("passwordStatus").textContent=pendingCreatedPassword?"방을 만든 직후라 비밀번호가 자동으로 준비되었습니다.":"";
  $("passwordModal").style.display="flex";
  if(!pendingCreatedPassword)$("roomPassword").focus();
}
...
async function prepareJoin(){
  ...
  if(!me||!room){setJoinStatus("닉네임과 방 이름을 입력하세요.");return}
  if(me.length>24||room.length>40){setJoinStatus("입력 길이를 확인하세요.");return}
  if(isProtectedNameClient(me)&&!$("adminPassword").value){setJoinStatus("이 닉네임은 관리자 비밀번호가 필요합니다.");$("adminPassword").focus();return}
  setJoinStatus("방 정보를 확인하는 중...");
  try{
    const r=await fetch("/api/room-exists?room="+encodeURIComponent(room),{cache:"no-store"});
    let d={};try{d=await r.json()}catch{}
    if(!r.ok||!d.exists)throw new Error(d.error||"존재하지 않는 방입니다.");
    openPasswordModal();
  }catch(e){setJoinStatus("방 입장 실패: "+e.message)}
}

async function confirmJoinPassword(){
  const password=$("roomPassword").value;
  if(!password){$("passwordStatus").textContent="방 비밀번호를 입력하세요.";return}
  $("passwordStatus").textContent="비밀번호 확인 중...";
  try{
    roomToken=await loginRoom(room,password);
    pendingCreatedPassword="";
    closePasswordModal();
    adminToken="";token="";clearConnectionState();manuallyLeft=false;
    if(isProtectedNameClient(me)){
      const adminPassword=$("adminPassword").value;
      setJoinStatus("관리자 인증 중...");
      try{adminToken=await loginAdmin(adminPassword)}catch(e){setJoinStatus("관리자 인증 실패: "+e.message);return}
    }
    connect();
...
async function searchRooms(){
  const q=$("roomSearchInput").value.trim(),box=$("roomList");
  ...
  try{
    ...
    if(!d.rooms?.length){box.innerHTML='<div class="empty">검색 결과가 없습니다.</div>';return}
  ...
  }catch(e){box.innerHTML='<div class="empty">방 목록 검색 실패: '+esc(e.message)+'</div>'}
}
function selectRoom(name){$("room").value=name;$("room").focus()}
...
async function loginAdmin(password){
  ...
  let d={};try{d=await r.json()}catch{}
  if(!r.ok)throw new Error(d.error||"관리자 인증에 실패했습니다.");
  if(!d.token)throw new Error("관리자 인증 토큰을 받지 못했습니다.");
  return d.token;
}
async function loginRoom(roomName,password){
  const r=await fetch("/api/room-login",{method:"POST",headers:{"content-type":"application/json"},cache:"no-store",body:JSON.stringify({room:roomName,password})});
  let d={};try{d=await r.json()}catch{}
  if(!r.ok)throw new Error(d.error||"방 비밀번호가 올바르지 않습니다.");
  if(!d.token)throw new Error("방 인증 토큰을 받지 못했습니다.");
  return d.token;
}
...
function send(){
  ...
  ws.send(JSON.stringify({type:"message",token,text}));input.value="";
}
function leave(){
  manuallyLeft=true;connecting=false;clearTimeout(reconnectTimer);reconnectTimer=null;
  const currentWs=ws;ws=null;roomToken="";pendingCreatedPassword="";
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health") {
      return json({ ok: true, service: "easy-second-chat", version: 7 });
    }
    ...
    if (url.pathname === "/api/room-exists" && request.method === "GET") {
      const room = cleanText(url.searchParams.get("room"), MAX_ROOM);
      if (!room || room === DIRECTORY_ROOM_ID) {
        return json({ ok:false, exists:false, error:"방 이름이 필요합니다." }, 400);
      }

      const id = env.CHAT_ROOM.idFromName(room);
      const roomStub = env.CHAT_ROOM.get(id);
      return roomStub.fetch(new Request(new URL("/room-exists", request.url)));
    }

    // 새 방 생성은 별도 API로 분리합니다. 기존 방의 로그인 요청으로 새 방이 생기지 않습니다.
    if (url.pathname === "/api/room-create" && request.method === "POST") {
      ...
      if (password.length < MIN_NEW_ROOM_PASSWORD) {
        return json({ ok:false, error:`새 방 비밀번호는 최소 ${MIN_NEW_ROOM_PASSWORD}자 이상으로 설정하세요.` }, 400);
      }
      ...
      if (!getRoomAuthSecret(env)) {
        return json({ ok:false, error:"방 인증용 Secret이 없습니다. ROOM_AUTH_SECRET 또는 ADMIN_PASSWORD를 설정하세요." }, 503);
      }

      const rate = await checkLoginRateLimit(env, request, "room-create");
      if (!rate.allowed) {
        return json({
          ok:false,
          error:`방 생성 시도가 너무 많습니다. ${rate.retryAfterSec}초 후 다시 시도하세요.`
        }, 429);
      }

      const id = env.CHAT_ROOM.idFromName(room);
      const roomStub = env.CHAT_ROOM.get(id);
      const response = await roomStub.fetch(new Request(new URL("/room-create", request.url), {
        method:"POST",
        headers:{"content-type":"application/json"},
      ...
      if (response.ok) {
        await clearLoginRateLimit(env, "room-create", clientIp(request));
      }
      return response;
    }
...
export class ChatRoom extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/room-exists" && request.method === "GET") {
      ...
      const createdAt = Number(await this.ctx.storage.get("roomCreatedAt"));
      const expiresAt = createdAt ? createdAt + ROOM_RESET_MS : 0;

      if (!roomName || !savedHash || !createdAt || expiresAt <= Date.now()) {
    ...
    if (url.pathname === "/room-create" && request.method === "POST") {
      ...
      const existingHash = await this.ctx.storage.get("roomPasswordHash");
      const existingCreatedAt = Number(await this.ctx.storage.get("roomCreatedAt"));
      const existingRoomName = await this.ctx.storage.get("roomName");

      if (existingHash && existingRoomName && existingCreatedAt && existingCreatedAt + ROOM_RESET_MS > Date.now()) {
        return json({ ok:false, error:"이미 존재하는 방입니다. 방 들어가기를 이용하세요." }, 409);
      }

      // 만료된 잔여 데이터를 정리한 뒤 새 방으로 초기화합니다.
      if (existingHash || existingRoomName || existingCreatedAt) {
        for (const ws of this.ctx.getWebSockets()) {
          try { ws.close(4000, "만료된 방 초기화"); } catch {}
        }
      ...
      return json({ ok:true, room:roomName, createdAt, expiresAt:createdAt + ROOM_RESET_MS });
    }
    ...
    if (url.pathname === "/room-auth" && request.method === "POST") {
      ...
      if (!savedHash) {
        return json({ ok:false, error:"존재하지 않는 방입니다. 먼저 방 만들기를 이용하세요." }, 404);
      } else {
        let valid = false;
