...
async function createRoomToken(room, env) {
  if (!env.ROOM_AUTH_SECRET) return null;

  const exp = Date.now() + ROOM_TOKEN_TTL_MS;
  const nonceBytes = new Uint8Array(18);
  crypto.getRandomValues(nonceBytes);

  const payload = `${room}.${exp}.${bytesToBase64Url(nonceBytes)}`;
  const signature = await hmacSign(payload, env.ROOM_AUTH_SECRET);

  return `${exp}.${bytesToBase64Url(nonceBytes)}.${signature}`;
}

async function verifyRoomToken(token, room, env) {
  if (!env.ROOM_AUTH_SECRET || !token) return false;
  ...
  const payload = `${room}.${expText}.${nonce}`;
  const expected = await hmacSign(payload, env.ROOM_AUTH_SECRET);
...
<div class="sub">방 이름과 비밀번호가 모두 맞아야 입장할 수 있습니다.</div>
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/room-login" && request.method === "POST") {
      ...
      if (!env.ROOM_AUTH_SECRET) {
        return json({ ok:false, error:"서버에 ROOM_AUTH_SECRET이 설정되지 않았습니다." }, 503);
      }
      ...
      return roomStub.fetch(authRequest);
    }
...
export class ChatRoom extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/room-auth" && request.method === "POST") {
      ...
      if (!savedHash) {
        await this.ctx.storage.put("roomPasswordHash", passwordHash);
      } else if (!safeEqual(String(savedHash), passwordHash)) {
        return json({ ok:false, error:"방 비밀번호가 올바르지 않습니다." }, 401);
      }

      const roomToken = await createRoomToken(this.ctx.id.toString(), this.env);
      if (!roomToken) {
    ...
    const validRoomToken = await verifyRoomToken(roomToken, this.ctx.id.toString(), this.env);
    if (!validRoomToken) {
