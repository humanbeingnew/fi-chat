...
<input id="name" maxlength="24" placeholder="닉네임">
...
<script>
let ws=null, me="", room="", token="", reconnectTimer=null, manuallyLeft=false;
...
function connect(){
 ...
 ws.onmessage=e=>{
   ...
   if(d.type==="init"){
     ...
     addMsg({system:true,time:d.time||"",text:d.message});
   }
 };
 ...
 ws.onclose=e=>{
   ...
   status("WebSocket 연결 종료: 코드 "+e.code+reason+" (3초 후 재연결)");
   clearTimeout(reconnectTimer);
   reconnectTimer=setTimeout(connect,3000);
 };

 ws.onerror=()=>status("WebSocket 연결 오류. 3초 후 다시 시도합니다...");
}
...
function send(){const input=$("text"),text=input.value.trim();if(!text||!ws||ws.readyState!==1)return;ws.send(JSON.stringify({type:"message",token,text}));input.value=""}
function leave(){manuallyLeft=true;clearTimeout(reconnectTimer);if(ws)ws.close();location.reload()}
</script></body></html>`;
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health") return json({ ok: true, service: "easy-second-chat", version: 2 });
    ...
    if (url.pathname === "/ws") {
      ...
      if (!room || !name) return json({ ok:false, error:"room/name required" }, 400);
      if (containsProfanity(name)) return json({ ok:false, error:"사용할 수 없는 닉네임입니다." }, 400);
...
export class ChatRoom extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (this.ctx.getWebSockets().length >= MAX_MEMBERS) return json({ ok:false, error:"방 인원이 가득 찼습니다." }, 429);
    ...
    const session = { id: crypto.randomUUID(), name, token, joinedAt: Date.now(), rate: [] };

    this.ctx.acceptWebSocket(server, ["chat"]);
    server.serializeAttachment(session);

    const members = this.members();
    server.send(JSON.stringify({ type:"init", token, members, messages: await this.history() }));
  ...
  members() {
    return this.ctx.getWebSockets().map(ws => ws.deserializeAttachment()?.name).filter(Boolean);
  }
  ...
  async webSocketMessage(ws, raw) {
    ...
    if (!s) return;
    let data;
    try { data = JSON.parse(raw); } catch { ws.send(JSON.stringify({type:"error",message:"잘못된 요청입니다."})); return; }
    if (data.token !== s.token) { ws.send(JSON.stringify({type:"error",message:"인증 실패"})); return; }

    if (data.type !== "message") return;
    const now = Date.now();
    s.rate = s.rate.filter(t => now - t < RATE_WINDOW_MS);
    if (s.rate.length >= MAX_MESSAGES_PER_WINDOW) {
      ws.send(JSON.stringify({type:"error",time:timeNow(),message:"메시지를 너무 빠르게 보내고 있습니다."}));
    ...
    if (containsProfanity(text)) {
      ws.send(JSON.stringify({type:"error",time:timeNow(),message:"욕설이 포함된 메시지는 전송할 수 없습니다."}));
      ws.serializeAttachment(s);
      return;
    }

    const message = { type:"message", name:s.name, text, time:timeNow() };
  ...
  system(text) {
    const message = { type:"system", system:true, text, time:timeNow() };
  ...
  broadcast(data) {
    ...
    for (const ws of this.ctx.getWebSockets()) {
      try { if (ws.readyState === WebSocket.OPEN) ws.send(raw); } catch {}
    }
  }
  ...
  webSocketClose(ws) {
    const s = ws.deserializeAttachment();
    if (!s) return;
    this.broadcast({type:"system",system:true,text:`${s.name}님이 나갔습니다.`,time:timeNow()});
